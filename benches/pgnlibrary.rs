...
fn bench_parse_array(b: &mut Bencher) {
    b.iter(|| test::black_box(SPNDEF.parse_message(&MSG as &[u8; 8]).unwrap()))
}
#[bench]
fn bench_parse_message(b: &mut Bencher) {
    b.iter(|| test::black_box(SPNDEF.parse_message(&MSG[..]).unwrap()))
}
#[bench]
fn bench_parse_message_closure(b: &mut Bencher) {
    let parse = SPNDEF.parser();

    b.iter(|| test::black_box(parse(&MSG[..]).unwrap()))
}
...
fn bench_parse_canframe(b: &mut Bencher) {
    ...
    b.iter(|| test::black_box(SPNDEF.parse_message(&frame).unwrap()))
}
...
fn bench_parse_canframe_closure(b: &mut Bencher) {
    ...
    b.iter(|| test::black_box(parse(&FRAME as &CANFrame).unwrap()))
}
